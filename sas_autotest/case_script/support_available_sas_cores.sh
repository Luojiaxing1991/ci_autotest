...
function FIO_IO_read_write()
{
    ...
    IO_read_write
    [ $? -eq 1 ] && MESSAGE="FAIL\tFIO tool read and write disk failure." && return 1
    MESSAGE="PASS"
}
...
function FIO_IO_RAIO_randrw()
{
    ...
    do
        ...
        fio fio.conf
        [ $? -ne 0 ] && MESSAGE="FAIL\tfio tool ${ratio} raio mixed io read and write failed." && return 1
    done
    MESSAGE="PASS"
}
...
function FIO_IO_RAIO_read_write()
{
    ...
    do
        ...
        fio fio.conf
        [ $? -ne 0 ] && MESSAGE="FAIL\tfio tool ${rw} io read and write failed." && return 1
    done
