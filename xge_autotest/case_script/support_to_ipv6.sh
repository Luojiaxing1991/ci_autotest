...
function ge_set_ipv6_address()
{
    ...
    ssh root@${BACK_IP} "ping6 ${local_tp1_ipv6_ip}%${local_tp1} -c 5" > ${HNS_TOP_DIR}/data/log/set_ipv6_address.txt &
    cat ${HNS_TOP_DIR}/data/log/ipv6_ping_pack.txt | grep "received, 0% packet loss" >/dev/null
    if [ $? -eq 0 ];then
...
function xge_set_ipv6_address()
{
    ...
    ssh root@${BACK_IP} "ping6 ${local_fibre1_ipv6_ip}%${local_fibre1} -c 5" > ${HNS_TOP_DIR}/data/log/set_ipv6_address.txt &
    cat ${HNS_TOP_DIR}/data/log/ipv6_ping_pack.txt | grep "received, 0% packet loss" >/dev/null
    if [ $? -eq 0 ];then
