...
function ge_iperf_set_mtu_value()
{
    ...
    iperf -c ${remote_tp1} -t 3600 -i 2 -P 3 > ${HNS_TOP_DIR}/data/logiperf_set_mtu_value.txt &
    ...
    do
        ...
        do
            ...
            NewMtuValue=$(ifconfig eth1 | grep "MTU" | awk '{print $(NF-1)}' | awk -F':' '{print $NF}')
            bandwidth=$(cat ${HNS_TOP_DIR}/data/logiperf_set_mtu_value | tail -1 | awk '{print $(NF-1)}')
...
function xge_iperf_set_mtu_value()
{
    ...
    iperf -c ${remote_fibre1} -t 3600 -i 2 -P 3 > ${HNS_TOP_DIR}/data/logiperf_set_mtu_value.txt &
    valuelist="68 1500 9706"
    for ((i=1;i<=100;i++));
    do
        for value in $valuelist
